...
const formatVariationName = (variation: string): string => {
  ...
  const patterns = [
    // Pattern pour [1630345] Variation 1
    /\[.*\]\s*Variation\s*(\d+)/,
    // Pattern pour BR#3_included_var1
    /var(\d+)/i,
    // Pattern pour variation1, variation2, etc.
    /variation\s*(\d+)/i,
    // Pattern pour var_1, var_2, etc.
    /var_(\d+)/i,
    // Pattern pour v1, v2, etc.
    /v(\d+)/i
  ];
...
export function RevenueRangeTable({ 
  ...
  useEffect(() => {
    if (quantityChartType === "correlation") {
      // Réinitialiser d'abord l'animation
      setShowAnimation(false);
      
      // Déclencher l'animation après un délai plus long
      ...
      // Réinitialiser l'animation quand on quitte l'onglet correlation
      setShowAnimation(false);
    }
  }, [quantityChartType]);
